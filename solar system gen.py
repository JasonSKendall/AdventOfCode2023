...
#  https://gist.github.com/chongchonghe
...
sim_start_date = "2029-03-11"     # simulating a solar system starting from this date
sim_duration = 6 * 365                # (int) simulation duration in days
...
class Object:                   # define the objects: the Sun, Earth, Mercury, etc
    def __init__(self, name, rad, color, r, v):
        ...
        self.line, = ax.plot([], [], color=color, linewidth=0.9)
...
class SolarSystem:
    def __init__(self, thesun):
        ...
        self.planets = []
        self.time = None
        self.timestamp = ax.text(.03, .94, 'Date: ', color='w', transform=ax.transAxes, fontsize='large')
    ...
    def evolve(self):           # evolve the trajectories
        ...
        for p in self.planets:
            ...
            plots.append(p.plot)
            lines.append(p.line)
        self.timestamp.set_text('Date: ' + self.time.iso.split()[0])
...
ax.set_aspect('equal')
ax.axis('off')
ss = SolarSystem(Object("Sun", 38, 'gold', [0, 0, 0], [0, 0, 0]))
ss.time = Time(sim_start_date)
ss.time.format='jd'
colors = ['gray', 'orange', 'blue', 'orangered']
...
f = r"./solar_system_16in_150dpi.mov"
writervideo = animation.FFMpegWriter(fps=60) 
ani.save(f, writer=writervideo, dpi=150)
